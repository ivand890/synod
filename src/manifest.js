...
const ownershipValues = new Set(["synod", "shared", "user", "record"]);
...
export function createManifest({ templateVersion, profile, files, migrations = [], schemaVersion = MANIFEST_SCHEMA_VERSION }) {
  return {
    schemaVersion,
    templateVersion,
    method: "advisor-loop",
...
export function validateManifest(manifest, { allowLegacy = true } = {}) {
  ...
  if (manifest.schemaVersion < MANIFEST_SCHEMA_VERSION && allowLegacy) {
...
function validateSchema2Manifest(manifest) {
  if (!parseVersion(manifest.templateVersion) || typeof manifest.profile !== "string") {
    throw new SynodError(ERROR_CODES.MANIFEST_INVALID, "Manifest is missing templateVersion or profile.");
  }
  if (manifest.hashAlgorithm !== "sha256" || !Array.isArray(manifest.files)) {
  ...
  for (const entry of manifest.files) {
    if (
      !entry || typeof entry.path !== "string" || !allowedManifestPath(entry.path)
      || seen.has(entry.path) || !["synod", "shared", "user"].includes(entry.ownership)
