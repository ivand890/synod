...
If project rotation is configured, run `__SYNOD_COMMAND__ rotation report` for read-only phase guidance. Use `rotation prepare` only after a configured threshold actually crosses; it records a recommendation and emits the exact handoff boundary without changing tasks, Git, or Codex threads. Start a fresh root session from that handoff, then run `rotation verify --root-session <id>` there to bind the new phase. Do not reuse an old or descendant session, and do not treat missing context evidence as a threshold crossing.

Use `__SYNOD_COMMAND__ usage --price-file <path>` only when a reviewed local price file is available and monetary projection is useful. The file must name exact models and a valid date window. Synod does not discover prices or contact a pricing service; incomplete usage or an unpriced model intentionally suppresses the grand total.
