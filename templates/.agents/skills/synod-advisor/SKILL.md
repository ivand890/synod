...
If project rotation is configured, run `__SYNOD_COMMAND__ rotation report` for read-only phase guidance. Use `rotation prepare` only after a configured threshold actually crosses; it records a recommendation and emits the exact handoff boundary without changing tasks, Git, or Codex threads. Start a fresh root session from that handoff, then run `rotation verify --session <id>` there to bind the new phase. Do not reuse an old or descendant session, and do not treat missing context evidence as a threshold crossing.
