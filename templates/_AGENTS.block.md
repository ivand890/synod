...
- When project rotation is configured, use `__SYNOD_COMMAND__ rotation report` for read-only guidance. Run `rotation prepare` only after an actual threshold crossing and `rotation verify` only from the exact fresh root session named by the prepared handoff; both are explicit canonical mutations. Optional `usage --price-file <path>` estimates cost from a reviewed local price file and never fetches prices from the network.
